...
def set_data(): #Function to convert from raw to real values
    gyrox = 4 * read_data(gyro_x_addr) / 131
    gyroy = 4 * read_data(gyro_y_addr) / 131
    gyroz = 4 * read_data(gyro_z_addr) / 131
...
def my_callback(): #Callback for button press to trigger and save data
    ...
    i = 0
    while os.path.exists("imuData%03dPOLL.csv" % i):
        i += 1
    np.savetxt("imuData%03dPOLL.csv" % i, data2, delimiter=",") #save truncated data
    #print(time.time() - starttime)    
    print("File imuData%03dPOLL.csv saved" % i)
    go = True
...
c = bus.read_byte_data(address, 0x1b)
c = c | 2 << 3
bus.write_byte_data(address, 0x1b, c) #Set Gyro Scale Range to 500 deg/s (32.8lsb/s)
